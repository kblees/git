...
{
	trace_performance(start_time, "command: %s", GetCommandLineA());
}
 ...
 * If t is positive, it is assumed to be the absolute time to print in seconds.
 * If negative, it is assumed to be the start of measurement and the current
 * ticks() are added before printing.
 */
__attribute__((format (printf, 4, 5)))
void trace_performance_file_line(const char *file, int lineno, double t,
		const char *fmt, ...)
{
	struct strbuf buf = STRBUF_INIT;
	va_list args;
