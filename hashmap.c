...
/* grow if > 66% full (to ~16%) */
#define HASHMAP_GROW_AT 1.5
/* shrink if < 5% full (to ~20%) */
#define HASHMAP_SHRINK_AT 20

static inline int entry_equals(const hashmap *map, const hashmap_entry *e1,
		const hashmap_entry *e2)
{
	return (e1 == e2) || (e1->hash == e2->hash && (*map->cmpfn)(e1, e2) == 0);
}
...
void hashmap_init(hashmap *map, hashmap_cmp_fn compare_function, size_t initial_size)
{
	map->size = 0;
	map->cmpfn = compare_function;
